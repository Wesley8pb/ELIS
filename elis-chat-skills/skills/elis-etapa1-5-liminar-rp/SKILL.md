---
name: elis-etapa1-5-liminar-rp
description: Etapa 1.5 do fluxo ELIS — análise e elaboração de decisões sobre pedidos de tutela de urgência (liminares) em representações de propaganda eleitoral irregular (Classe RP). Cobre três cenários — concessão, indeferimento e concessão parcial da liminar — conforme Lei nº 9.504/97, Resoluções do TSE e aplicação subsidiária do CPC (art. 300). Acionar quando detectada classe processual "RP" com pedido de tutela de urgência, ou quando o usuário disser "execute a tutela de urgência", "analise o pedido liminar" ou usar analise o pedido liminar. Fluxo independente — pode rodar a qualquer momento, antes ou depois de qualquer etapa. Utilizar em conjunto com o arquivo reference.md complementar.
---
...
Salvar a decisão em `CONTEXTO/ETAPA1.5-LIMINAR-RP.md` na pasta de trabalho atual (criar a pasta `CONTEXTO/` se não existir).

## AO CONCLUIR ESTA ETAPA — manter o fluxo ativo

Após salvar a decisão e apresentá-la ao usuário:

1. Perguntar se deseja ajustes na decisão liminar.
2. Sugerir explicitamente a próxima fase do fluxo ELIS: **"Deseja prosseguir para a Etapa 2 (Deliberação — `prossiga para a Etapa 2`) ou ir direto para a Etapa 3 (Arquitetura da Sentença — `prossiga para a Etapa 3`)?"**
3. Nunca encerrar a conversa sem oferecer a próxima etapa.
...
**Versão:** 2.1
**Skill:** elis-etapa1-5-liminar-rp (fluxo ELIS)
