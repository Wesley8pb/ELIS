...
Os scripts ficam em `${CLAUDE_PLUGIN_ROOT}/skills/conversao-arquivos/scripts/`:
...
```bash
# PDF (requer pypdf)
python3 "${CLAUDE_PLUGIN_ROOT}/skills/conversao-arquivos/scripts/extract_pdf_to_md.py" "processo.pdf" "processo - PROCESSADO.md"

# DOCX (sem dependências externas)
python3 "${CLAUDE_PLUGIN_ROOT}/skills/conversao-arquivos/scripts/extract_docx_to_md.py" "documento.docx" "documento - PROCESSADO.md"
```

### Windows (PowerShell)

```powershell
powershell -ExecutionPolicy Bypass -File "${CLAUDE_PLUGIN_ROOT}/skills/conversao-arquivos/scripts/prepare-input.ps1" -InputFile "processo.pdf"
